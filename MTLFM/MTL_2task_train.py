...
from LoadData import IPD_MA_MTL_for2,Load_saved_data_MTLfor2
from Util import EarlyStopper,seed_torch,get_model,prob_to_label
from train_test_framework import train_2task,test_2task
...
def main(task_number,
    ...
    data = IPD_MA_MTL_for2(dataset_path,task_number)
    ...
    # dataset_path = './data/'
    # training_set = Load_saved_data_MTLfor2(dataset_path+'training_set.npy',[1,2])
    # validation_set = Load_saved_data_MTLfor2(dataset_path+'validation_set.npy',[1,2])
    # test_set = Load_saved_data_MTLfor2(dataset_path+'test_set.npy',[1,2])
    ...
    model_log = open(f'{save_dir}/2task_model_log.txt','a') 
    ...
    for epoch_i in range(epoch):
        train_2task(model, optimizer, train_data_loader, criterion, device)
        total_target,total_predict,loss = test_2task(model, valid_data_loader, device)
    ...
    total_target,total_predict,logloss= test_2task(original_model, test_data_loader, device)
    print(f'test total auc: {roc_auc_score(total_target[0], total_predict[0])},task1 auc: {roc_auc_score(total_target[1], total_predict[1])},task2 auc: {roc_auc_score(total_target[2], total_predict[2])}')
    ...
    print(f'test total loss: {logloss[0]},task1 loss: {logloss[1]},task2 loss: {logloss[2]}')
    model_log.flush()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task_number', default=[1,2])
    parser.add_argument('--dataset_path', default='./data/cutoff_75%_training_data.npy')
    parser.add_argument('--model_name', default='mtlnfm_for2')
